 ...
 */
const getPath = (type) => path.join(__dirname, '../tmp', `${type}.cron_lock`);
...
const lock = (type) =>
{
  ...
  if (found)
  {
    throw new Error(`Lock found for '${ type}', cron may already be running. If not already running remove ${ type}.cron_lock and try again.`);
  }
 ...
 * @param {String} type The cron name.
 */
const unlock = (type) => fs.unlinkSync(getPath(type));
